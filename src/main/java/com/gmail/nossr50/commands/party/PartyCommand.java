...
public class PartyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 && !PP.inParty()) {
             ...
             * if(Party.getInstance().isPartyLeader(a.getName(), PP.getParty())
             * && !a.getName().equals(player.getName())) { PlayerProfile LP =
             * Users.getProfile(a); for(SkillType type : SkillType.values()) {
             * //Skip this one if(type == SkillType.ALL) continue;
             *
             * if(LP.getSkillLevel(type) > PP.getSkillLevel(type)) { //Tell them
             * what their skill bonus is for this skill int leaderLevel =
             * LP.getSkillLevel(type); int difference = leaderLevel -
             * PP.getSkillLevel(type);
             *
             * double bonusModifier = (difference*0.75D)/100D; double trueBonus
             * = bonusModifier * 100; player.sendMessage("You get "+trueBonus+"%
             * more XP from "+type.toString()); //double percent =
             * (trueBonus/100)*100; } else { //Tell them they have no bonus.. or
             * not } } } else if(Party.getInstance().isPartyLeader(a.getName(),
             * PP.getParty()) && a.getName().equals(player.getName())) { //Tell
             * them they are providing bonuses } }
             */
